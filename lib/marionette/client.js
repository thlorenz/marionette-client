...
function Client(driver, options) {
  ...
  if (!driver && options && options.lazy) return
...
Client.prototype = {
  ...
  plugin: function (name, plugin, options) {
    ...
    if (this[name]) throw new Error(name + ' - is already reserved on client')

    // allow both plugin.setup and plugin to be the invoked method.
    invokedMethod = typeof plugin.setup === 'function' ? plugin.setup : plugin
    ...
    // plugin.
    if (name && result) this[name] = result
  ...
  send: function send(body, cb) {
    ...
    try {
      ...
      if (driverSent instanceof Promise) {
        driverSent.then(function onFulfill(res) {
          cb(res)
        })
      }
    } catch (e) {
  ...
  _handleCallback: function () {
    ...
    assert(typeof callback == 'function', 'expected function, got ' + callback)
  ...
  sessionCapabilities: function sessionCapabilities(callback) {
    ...
    return this._sendCommand(
      cmd,
      callback,
      this.protocol == 1 ? 'value' : 'capabilities'
    )
  },
  ...
  getWindows: function getWindows(callback) {
    ...
    return this._sendCommand(
      cmd,
      callback,
      this.protocol == 1 ? 'value' : undefined
    )
  },
  ...
  switchToFrame: function switchToFrame(id, options, callback) {
    ...
    } else if (id !== null && typeof id === 'object' && id.ELEMENT) {
