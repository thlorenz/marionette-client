...
suite('marionette/client', function () {
  var subject, driver, cb, cbResponse, device, Element, Client, Exception
  ...
  suite('.plugin', function () {
    test('should allow chaining', function () {
      ...
      subject.plugin('one', pluginOne).plugin('two', pluginTwo)
  ...
  suite('.startSession protocol version 2', function () {
    ...
    setup(function (done) {
      ...
      device.withProtocol(2).shouldSend({
  ...
  suite('._sendCommand', function () {
    var cmd, response, calledUnmarshal, result, calledWith
    ...
    function receivesElement() {
      ...
      suite('callback argument', function () {
        setup(function () {
          ...
          if (value instanceof Array) els = value
          else els = [value]
        })
    ...
    suite("trying to find with invalid 'using'", function () {
      test('should fail', function () {
        assert.throws(function () {
          subject._findElement('findElement', 'wow', 'fake', function () {})
    ...
    function delegatesToFind(type) {
      suite('.' + type, function () {
        ...
        test('should call _findElement', function () {
          assert.deepEqual(calledWith, [type, '#query', commandCallback])
        })
      })
    }
