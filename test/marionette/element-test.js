...
suite('marionette/element', function () {
  var driver, subject, client, id, device, Element, Client
  ...
  suite('.scriptWith', function () {
    ...
    test(
      'should call client.executeScript with ' + 'element as argument',
      function () {
        subject.scriptWith(fn)

        assert.strictEqual(calledWith[0], fn)
        assert.deepEqual(calledWith[1], [subject])
        assert.strictEqual(calledWith[2], undefined)
      }
    )

    test(
      'should call client.executeScript with element ' + ' + callback',
      function () {
    ...
    test(
      'should call client.executeScript with element ' + ' + args + callback',
      function () {
        subject.scriptWith(fn, ['foo', 'bar'], cb)

        assert.strictEqual(calledWith[0], fn)
        assert.deepEqual(calledWith[1], [subject, 'foo', 'bar'])
        assert.strictEqual(calledWith[2], cb)
      }
    )
  })
